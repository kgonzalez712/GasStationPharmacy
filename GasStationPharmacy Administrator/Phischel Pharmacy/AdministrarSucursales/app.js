...
//método que llama a un post de una sucursal
    $scope.addBranchOff = function(){
		$http({
			method: 'POST',
			url:'http://' + $scope.ip + '/GasStationPharmacyWS/api/BranchOffices/Phischel/NewBranch',
			data: $scope.branch
...
//método que llama a un get de sucursales para ingresar estas a la tabla donde el usuario podrá elegir una a modoficar
    $scope.modifyBranch = function(){  
        $http.get('http://' + $scope.ip + '/GasStationPharmacyWS/api/BranchOffices/Phischel').
...
//método para modificar una sucursal, el cual hace un PUT al rest service.
    $scope.updateBranch = function(){
		$http({
			method: 'PUT',
			url: 'http://' + $scope.ip + '/GasStationPharmacyWS/api/BranchOffices/Phischel/' + $scope.search + '/UpdateBranch',
			data: $scope.branch
		}).then(function successCallback(response){
...
//método para eliminar una sucursal
	$scope.deleteBranchOff = function(branch){
		...
		$http({
			method: 'DELETE',
			url:'http://' + $scope.ip + '/GasStationPharmacyWS/api/BranchOffices/Phischel/' + $scope.branch.BranchOfficeName + '/DeleteBranch',
		}).then(function successCallback(response){
