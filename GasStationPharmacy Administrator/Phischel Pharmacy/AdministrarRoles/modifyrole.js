...
app.controller('myCtrl',function($scope,$http){
   ...
   //función para activar la pantalla para modificar un rol, entradas el rol a modificar
    ...
    $scope.modifyRoles = function(){
        $http.get('http://' + $scope.ip + '/GasStationPharmacyWS/api/Roles/Phischel').
...
//método que llama a un post de un rol
    $scope.addRole = function(){
		$http({
			method: 'POST',
			url:'http://' + $scope.ip + '/GasStationPharmacyWS/api/Roles/Phischel/NewRole',
			data: $scope.role
    ...
    $scope.updateRol = function(){
		$http({
			method: 'PUT',
			url: 'http://' + $scope.ip + '/GasStationPharmacyWS/api/Roles/Phischel/' + $scope.role.RoleName + '/UpdateRole',
			data: $scope.role
		}).then(function successCallback(response){
	...
	$scope.deleteRol = function(rol){
		...
		$http({
			method: 'DELETE',
			url:'http://' + $scope.ip + '/GasStationPharmacyWS/api/Roles/Phischel/' + $scope.rol.RoleName + '/DeleteRole',
		}).then(function successCallback(response){
